...
with open('../results/authorization.json') as f:
    data = json.load(f)
...
plt.ylabel('Average Authorization Time (ms)')
plt.title('Average Authorization Time vs Number of Delegations')
plt.legend()
...
plt.savefig('avg_auth_time_vs_nb_delegations.png')  # Change the extension to save in a different format
plt.show()
